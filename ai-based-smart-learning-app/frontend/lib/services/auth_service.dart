...
class AuthService {
  ...
  static Future<void> saveSession(Map<String, dynamic> user) async {
    ...
    await prefs.setString(_keyId,         user['id'].toString());
    await prefs.setString(_keyName,       (user['full_name'] ?? user['name'] ?? 'User').toString());
    await prefs.setString(_keyEmail,      user['email'].toString());
    await prefs.setString(_keyRole,       user['role'].toString());
    await prefs.setString(_keyDisability, (user['disability_mode'] ?? user['disability'] ?? 'none').toString());
  }
  ...
  static Future<String> getUserId()         async => (await getSession())?['id']         ?? '0';
